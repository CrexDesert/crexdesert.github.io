...
function checkAuth() {
    ...
    data.record.data.users.forEach( ( item, i ) => {
        ...
        if ( String(localStorage.getItem("NotepadAuthLogin")).toLocaleLowerCase() === String(item.login).toLocaleLowerCase() ) {
            document.querySelector('.main__avatar img').src = item.avatar;
        }

    })

    // alert(localStorage.getItem("NotepadAuthLogin"));
    // alert(localStorage.getItem("NotepadAuthLogin") == null);

    data.record.data.users.forEach( ( item, i ) => {
        if ( localStorage.getItem("NotepadAuthLogin") != null ) {
            if ( String(localStorage.getItem("NotepadAuthLogin")).toLocaleLowerCase() === String(item.login).toLocaleLowerCase() ) {
                exist = true;
            }
        }
...
function paintListUsers(data) {
    ...
    data.forEach( (item, i) => {
        ...
        newItem.classList.add("users__item");
        if ( item.status === "admin" ) {
            newItem.innerHTML = `<div class="users__picture">
                                <img src="${item.avatar}" alt="avatar">
                            </div>
                            <div class="users__info">
                                <div class="users__name">${item.login}</div>
                                <div class="users__status">Роль - Адмін</div>
                                <div class="users__mail">Пошта - ${item.mail}</div>
                                <div class="users__date">Дата реєстрації - ${item.date}</div>
                                <div class="users__msg">Кількість повідомлень - <span>0</span></div>
                            </div>`;
        } else {
            newItem.innerHTML = `<div class="users__picture">
                                <img src="${item.avatar}" alt="avatar">
                            ...
                            <div class="users__info">
                                <div class="users__name">${item.login}</div>
                                <div class="users__status">Роль - Користувач</div>
                                <div class="users__mail">Пошта - ${item.mail}</div>
                                <div class="users__date">Дата реєстрації - ${item.date}</div>
                                <div class="users__msg">Кількість повідомлень - <span>0</span></div>
                            </div>`;
    ...
    document.querySelectorAll('.users__num').forEach( (item, i) => {
        item.addEventListener('click', event => {

            document.querySelectorAll('.users__num').forEach( (item, i) => {
                item.classList.remove('users__num-active');
            })
            item.classList.add('users__num-active');
            ...
            if ( i == 0 ) {
                ...
                document.querySelectorAll('.users__status').forEach( (item, i) => {
                    if ( item.textContent == "Роль - Користувач" ) {
                        document.querySelectorAll('.users__item')[i].style.display = 'flex';
                    }
                })
            } else if ( i == 2 ) {
                document.querySelectorAll('.users__item').forEach( item => {
                ...
                document.querySelectorAll('.users__status').forEach( (item, i) => {
                    if ( item.textContent == "Роль - Адмін" ) {
                        document.querySelectorAll('.users__item')[i].style.display = 'flex';
                    }
                })
            }
    ...
    let countMsg = 0;
    let actualName = "";
    document.querySelectorAll('.users__name').forEach( (item, i) => {
        countMsg = 0;
        actualName = item.textContent;
        document.querySelectorAll('.wall__name').forEach( (item, i) => {
            if ( item.textContent == actualName ) {
                countMsg = countMsg + 1;
            }
        })
        document.querySelectorAll('.users__msg span')[i].textContent = countMsg;
    })
...
function addNewPost() {
    ...
    document.querySelector('.wall__save').style.display = "flex";
}

document.querySelector('.wall__save').addEventListener('click', event => {
    location.reload();
})
...
document.querySelector('.main__setting').addEventListener('click', event => {
    if ( activeAvatar == false ) {
        document.querySelector('.main__set').style.maxHeight = "140px";
        document.querySelector('.main__set').style.padding = "10px";
        document.querySelector('.main__setting').style.background = "#FFF";
        document.querySelector('.main__setting svg').style.fill = "#000";
        document.querySelector('.main__setting svg').style.transform = "rotate(90deg)";
        activeAvatar = true;
    } else {
        document.querySelector('.main__set').style.maxHeight = "0px";
        document.querySelector('.main__set').style.padding = "0px 10px";
        document.querySelector('.main__setting').style.background = "#000";
        document.querySelector('.main__setting svg').style.fill = "#fff";
        document.querySelector('.main__setting svg').style.transform = "rotate(0deg)";
        activeAvatar = false;
    }
})
...
document.querySelector('.main__exit').addEventListener('click', event => {
    document.querySelector('.proof').style.display = 'flex';
    setTimeout( () => {
        document.querySelector('.proof').style.opacity = '1';
    }, 100 )
})

document.querySelector('.proof__yes').addEventListener('click', event => {
    localStorage.setItem("NotepadAuth", "false");
    localStorage.clear();
    location.reload();
})

document.querySelector('.proof__no').addEventListener('click', event => {
    document.querySelector('.proof').style.opacity = '0';
    setTimeout( () => {
